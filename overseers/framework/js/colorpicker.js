...
(function (jQuery) {
	...
	jQuery.widget("vanderlee.colorpicker", {
		options: {
		...
		_create: function () {
			...
			if (this.element[0].nodeName.toLowerCase() === 'input') {
				...
				jQuery('body').append(_container_popup);
				self.dialog = jQuery('.ui-colorpicker:last');
				...
				jQuery(document).mousedown(function (event) {
					...
					if (jQuery(event.target).parents('.ui-colorpicker').length > 0) {
					...
					var p,
						parents = jQuery(event.target).parents();
					for (p in parents) {
				...
				if (self.options.showOn === 'button' || self.options.showOn === 'both') {
					...
					if (self.options.buttonImageOnly && self.image) {
						...
						self.button = jQuery('<button type="button"></button>').html(self.image || self.options.buttonText).button();
						self.image = self.image ? jQuery('img', self.button).first() : null;
					}
				...
				jQuery(this.element).html(_container_inline);
				self.dialog = jQuery('.ui-colorpicker', this.element);
		...
		_setAltField: function () {
			if (this.options.altField && this.options.altProperties) {
				...
				for (index in properties) {
					property = jQuery.trim(properties[index]);
					switch (property) {
						case 'color':
						case 'background-color':
						case 'outline-color':
						case 'border-color':
							jQuery(this.options.altField).css(property, this.color.toCSS());
							break;
					}
				}

				if (this.options.altAlpha) {
					jQuery(this.options.altField).css('opacity', this.color.a);
				}
			}
		},
		...
		_loadColor: function () {
			if(!this.color) {
				...
				this.currentColor = jQuery.extend({}, this.color);
			}
		...
		_generate: function () {
			...
			if (!self.generated) {
				...
				jQuery(self._layoutTable(layout_parts, function(cell, x, y) {
					var classes = [];
		...
		_effectGeneric: function (show, slide, fade, callback) {
			...
			if (jQuery.effects && jQuery.effects[self.options.showAnim]) {
		...
		open: function () {
			if (!this.opened) {
				...
				x -= Math.max(0, (x + this.dialog.width()) - jQuery(window).width() + 20);
				y -= Math.max(0, (y + this.dialog.height()) - jQuery(window).height() + 20);
		...
		close: function () {
			...
			self.currentColor	= jQuery.extend({}, self.color);
			self.changed		= false;
		...
		_generateAllParts: function () {
			jQuery.each(this.parts, function (index, part) {
				part.generate();
			});
		},
		...
		_parts: {
			header: function (inst) {
				...
				this.init = function () {
					e = jQuery(_html()).prependTo(inst.dialog);
					var close = jQuery('.ui-dialog-titlebar-close', e);
			...
			map: function (inst) {
				...
				_mousedown = function (event) {
					...
					var div		= jQuery('#ui-colorpicker-map-layer-pointer', e),
						offset	= div.offset(),
						width	= div.width(),
						height	= div.height(),
					...
					if (x >= 0 && x < width && y >= 0 && y < height) {
						...
						jQuery(document).unbind('mousedown', _mousedown);
						jQuery(document).bind('mouseup', _mouseup);
				...
				_mouseup = function (event) {
					event.stopImmediatePropagation();
					event.preventDefault();
					jQuery(document).unbind('mouseup', _mouseup);
					jQuery(document).unbind('mousemove', _mousemove);
					jQuery(document).bind('mousedown', _mousedown);
				};
				...
				this.generate = function () {
					switch (inst.mode) {
					case 'h':
						jQuery('#ui-colorpicker-map-layer-1', e).css({'background-position': '0 0', 'opacity': ''}).show();
						jQuery('#ui-colorcasepicker-map-layer-2', e).hide();
						break;

					case 's':
					case 'a':
						jQuery('#ui-colorpicker-map-layer-1', e).css({'background-position': '0 -260px', 'opacity': ''}).show();
						jQuery('#ui-colorpicker-map-layer-2', e).css({'background-position': '0 -520px', 'opacity': ''}).show();
						break;

					case 'v':
						jQuery(e).css('background-color', 'black');
						jQuery('#ui-colorpicker-map-layer-1', e).css({'background-position': '0 -780px', 'opacity': ''}).show();
						jQuery('#ui-colorpicker-map-layer-2', e).hide();
						break;

					case 'r':
						jQuery('#ui-colorpicker-map-layer-1', e).css({'background-position': '0 -1040px', 'opacity': ''}).show();
						jQuery('#ui-colorpicker-map-layer-2', e).css({'background-position': '0 -1300px', 'opacity': ''}).show();
						break;

					case 'g':
						jQuery('#ui-colorpicker-map-layer-1', e).css({'background-position': '0 -1560px', 'opacity': ''}).show();
						jQuery('#ui-colorpicker-map-layer-2', e).css({'background-position': '0 -1820px', 'opacity': ''}).show();
						break;

					case 'b':
						jQuery('#ui-colorpicker-map-layer-1', e).css({'background-position': '0 -2080px', 'opacity': ''}).show();
						jQuery('#ui-colorpicker-map-layer-2', e).css({'background-position': '0 -2340px', 'opacity': ''}).show();
						break;
					}
					self.repaint();
				};
				...
				this.repaint = function () {
					...
					case 'h':
						...
						jQuery(e).css('background-color', inst.color.normClone().toCSS());
						break;
					...
					case 'a':
						...
						y = (1 - inst.color.v) * div.width();
						jQuery('#ui-colorpicker-map-layer-2', e).css('opacity', 1 - inst.color.s);
						break;

					case 'v':
						x = inst.color.h * div.width();
						y = (1 - inst.color.s) * div.width();
						jQuery('#ui-colorpicker-map-layer-1', e).css('opacity', inst.color.v);
						break;

					case 'r':
						x = inst.color.b * div.width();
						y = (1 - inst.color.g) * div.width();
						jQuery('#ui-colorpicker-map-layer-2', e).css('opacity', inst.color.r);
						break;

					case 'g':
						x = inst.color.b * div.width();
						y = (1 - inst.color.r) * div.width();
						jQuery('#ui-colorpicker-map-layer-2', e).css('opacity', inst.color.g);
						break;

					case 'b':
						x = inst.color.r * div.width();
						y = (1 - inst.color.g) * div.width();
						jQuery('#ui-colorpicker-map-layer-2', e).css('opacity', inst.color.b);
						break;
					}

					if (inst.options.alpha) {
						jQuery('#ui-colorpicker-map-layer-alpha', e).css('opacity', 1 - inst.color.a);
					}

					jQuery('#ui-colorpicker-map-pointer', e).css({
						'left': x - 7,
						'top': y - 7
					});
				};
				...
				this.init = function () {
					e = jQuery(_html()).appendTo(jQuery('#ui-colorpicker-map-container', inst.dialog));
			...
			bar: function (inst) {
				...
				_mousedown = function (event) {
					...
					var div		= jQuery('#ui-colorpicker-bar-layer-pointer', e),
						offset	= div.offset(),
						width	= div.width(),
						height	= div.height(),
						x		= event.pageX - offset.left,
				...
				this.generate = function () {
					...
					case 'g':
					case 'b':
						jQuery('#ui-colorpicker-bar-layer-alpha', e).show();
						jQuery('#ui-colorpicker-bar-layer-alphabar', e).hide();
						break;

					case 'a':
						jQuery('#ui-colorpicker-bar-layer-alpha', e).hide();
						jQuery('#ui-colorpicker-bar-layer-alphabar', e).show();
						break;
					}

					switch (inst.mode) {
					case 'h':
						jQuery('#ui-colorpicker-bar-layer-1', e).css({'background-position': '0 0', 'opacity': ''}).show();
						jQuery('#ui-colorpicker-bar-layer-2', e).hide();
						jQuery('#ui-colorpicker-bar-layer-3', e).hide();
						jQuery('#ui-colorpicker-bar-layer-4', e).hide();
						break;

					case 's':
						jQuery('#ui-colorpicker-bar-layer-1', e).css({'background-position': '0 -260px', 'opacity': ''}).show();
						jQuery('#ui-colorpicker-bar-layer-2', e).css({'background-position': '0 -520px', 'opacity': ''}).show();
						jQuery('#ui-colorpicker-bar-layer-3', e).hide();
						jQuery('#ui-colorpicker-bar-layer-4', e).hide();
						break;

					case 'v':
						jQuery('#ui-colorpicker-bar-layer-1', e).css({'background-position': '0 -520px', 'opacity': ''}).show();
						jQuery('#ui-colorpicker-bar-layer-2', e).hide();
						jQuery('#ui-colorpicker-bar-layer-3', e).hide();
						jQuery('#ui-colorpicker-bar-layer-4', e).hide();
						break;
					...
					case 'r':
						jQuery('#ui-colorpicker-bar-layer-1', e).css({'background-position': '0 -1560px', 'opacity': ''}).show();
						jQuery('#ui-colorpicker-bar-layer-2', e).css({'background-position': '0 -1300px', 'opacity': ''}).show();
						jQuery('#ui-colorpicker-bar-layer-3', e).css({'background-position': '0 -780px', 'opacity': ''}).show();
						jQuery('#ui-colorpicker-bar-layer-4', e).css({'background-position': '0 -1040px', 'opacity': ''}).show();
						break;

					case 'g':
						jQuery('#ui-colorpicker-bar-layer-1', e).css({'background-position': '0 -2600px', 'opacity': ''}).show();
						jQuery('#ui-colorpicker-bar-layer-2', e).css({'background-position': '0 -2340px', 'opacity': ''}).show();
						jQuery('#ui-colorpicker-bar-layer-3', e).css({'background-position': '0 -1820px', 'opacity': ''}).show();
						jQuery('#ui-colorpicker-bar-layer-4', e).css({'background-position': '0 -2080px', 'opacity': ''}).show();
						break;

					case 'b':
						jQuery('#ui-colorpicker-bar-layer-1', e).css({'background-position': '0 -3640px', 'opacity': ''}).show();
						jQuery('#ui-colorpicker-bar-layer-2', e).css({'background-position': '0 -3380px', 'opacity': ''}).show();
						jQuery('#ui-colorpicker-bar-layer-3', e).css({'background-position': '0 -2860px', 'opacity': ''}).show();
						jQuery('#ui-colorpicker-bar-layer-4', e).css({'background-position': '0 -3120px', 'opacity': ''}).show();
						break;

					case 'a':
						jQuery('#ui-colorpicker-bar-layer-1', e).hide();
						jQuery('#ui-colorpicker-bar-layer-2', e).hide();
						jQuery('#ui-colorpicker-bar-layer-3', e).hide();
						jQuery('#ui-colorpicker-bar-layer-4', e).hide();
						break;
				...
				this.repaint = function () {
					...
					case 's':
						...
						jQuery('#ui-colorpicker-bar-layer-2', e).css('opacity', 1 - inst.color.v);
						jQuery(e).css('background-color', inst.color.normClone().toCSS());
						break;
					...
					case 'r':
						...
						jQuery('#ui-colorpicker-bar-layer-2', e).css('opacity', Math.max(0, (inst.color.b - inst.color.g)));
						jQuery('#ui-colorpicker-bar-layer-3', e).css('opacity', Math.max(0, (inst.color.g - inst.color.b)));
						jQuery('#ui-colorpicker-bar-layer-4', e).css('opacity', Math.min(inst.color.b, inst.color.g));
						break;

					case 'g':
						y = (1 - inst.color.g) * div.height();
						jQuery('#ui-colorpicker-bar-layer-2', e).css('opacity', Math.max(0, (inst.color.b - inst.color.r)));
						jQuery('#ui-colorpicker-bar-layer-3', e).css('opacity', Math.max(0, (inst.color.r - inst.color.b)));
						jQuery('#ui-colorpicker-bar-layer-4', e).css('opacity', Math.min(inst.color.r, inst.color.b));
						break;

					case 'b':
						y = (1 - inst.color.b) * div.height();
						jQuery('#ui-colorpicker-bar-layer-2', e).css('opacity', Math.max(0, (inst.color.r - inst.color.g)));
						jQuery('#ui-colorpicker-bar-layer-3', e).css('opacity', Math.max(0, (inst.color.g - inst.color.r)));
						jQuery('#ui-colorpicker-bar-layer-4', e).css('opacity', Math.min(inst.color.r, inst.color.g));
						break;
					...
					if (inst.mode !== 'a') {
						jQuery('#ui-colorpicker-bar-layer-alpha', e).css('opacity', 1 - inst.color.a);
					}

					jQuery('#ui-colorpicker-bar-pointer', e).css('top', y - 3);
				};

				this.init = function () {
					e = jQuery(_html()).appendTo(jQuery('#ui-colorpicker-bar-container', inst.dialog));
			...
			inputs: function (inst) {
				...
				this.init = function () {
					e = jQuery(_html()).appendTo(jQuery('#ui-colorpicker-inputs-container', inst.dialog));

					jQuery('.ui-colorpicker-mode', e).click(function () {
						inst.mode = jQuery(this).val();
						inst._generateAllParts();
					});

					jQuery('.ui-colorpicker-number', e).bind('change input keyup', function () {
						inst.color.r = jQuery('#ui-colorpicker-r .ui-colorpicker-number', e).val() / 255;
						inst.color.g = jQuery('#ui-colorpicker-g .ui-colorpicker-number', e).val() / 255;
						inst.color.b = jQuery('#ui-colorpicker-b .ui-colorpicker-number', e).val() / 255;
						inst.color.a = jQuery('#ui-colorpicker-a .ui-colorpicker-number', e).val() / 100;
						inst.color.h = jQuery('#ui-colorpicker-h .ui-colorpicker-number', e).val() / 360;
						inst.color.s = jQuery('#ui-colorpicker-s .ui-colorpicker-number', e).val() / 100;
						inst.color.v = jQuery('#ui-colorpicker-v .ui-colorpicker-number', e).val() / 100;

						if (jQuery(this).hasClass('ui-colorpicker-number-hsv')) {
							inst.color.updateRGB();
						} else if (jQuery(this).hasClass('ui-colorpicker-number-rgb')) {
							inst.color.updateHSV();
						}
				...
				this.repaint = function () {
					var c = jQuery.extend(inst.color);
					c.h *= 360;
					c.s *= 100;
					c.v *= 100;
					c.r *= 255;
					...
					jQuery.each(c, function (index, value) {
						...
						if (!jQuery('#ui-colorpicker-' + index + ' .ui-colorpicker-number', e).is(':focus')
								&& jQuery('#ui-colorpicker-' + index + ' .ui-colorpicker-number', e).val() !== v) {
							jQuery('#ui-colorpicker-' + index + ' .ui-colorpicker-number', e).val(v);
						}
					});
				};
				...
				this.generate = function () {
					jQuery('.ui-colorpicker-mode', e).each(function () {
						jQuery(this).attr('checked', jQuery(this).val() === inst.mode);
					});
					this.repaint();
				};
			},
			...
			preview: function (inst) {
				...
				this.init = function () {
					e = jQuery(_html()).appendTo(jQuery('#ui-colorpicker-preview-container', inst.dialog));

					jQuery('#ui-colorpicker-preview-initial', e).click(function () {
						inst.color = jQuery.extend({}, inst.currentColor);
						inst._change();
					});
				...
				this.repaint = function () {
					jQuery('#ui-colorpicker-preview-initial', e).css('background-color', inst.currentColor.toCSS()).attr('title', inst.currentColor.toHex());
					jQuery('#ui-colorpicker-preview-initial-alpha', e).css('opacity', 1 - inst.currentColor.a);
					jQuery('#ui-colorpicker-preview-current', e).css('background-color', inst.color.toCSS()).attr('title', inst.color.toHex());
					jQuery('#ui-colorpicker-preview-current-alpha', e).css('opacity', 1 - inst.color.a);
				};

				this.generate = function () {
					if (inst.options.alpha) {
						jQuery('#ui-colorpicker-preview-initial-alpha, #ui-colorpicker-preview-current-alpha', e).show();
					} else {
						jQuery('#ui-colorpicker-preview-initial-alpha, #ui-colorpicker-preview-current-alpha', e).hide();
					}
			...
			hex: function (inst) {
				...
				this.init = function () {
					e = jQuery(_html()).appendTo(jQuery('#ui-colorpicker-hex-container', inst.dialog));

					jQuery('#ui-colorpicker-hex-input', e).bind('change keyup', function () {
						var rgb = inst._parseHex(jQuery(this).val());
						inst.color.r = rgb[0];
						inst.color.g = rgb[1];
						inst.color.b = rgb[2];
						inst.color.updateHSV();
					...
					jQuery('#ui-colorpicker-hex-alpha', e).bind('change keyup', function () {
						inst.color.a = parseInt(jQuery('#ui-colorpicker-hex-alpha', e).val(), 16);
						inst._change();
					});
				};

				this.repaint = function () {
					if (!jQuery('#ui-colorpicker-hex-input', e).is(':focus')) {
						jQuery('#ui-colorpicker-hex-input', e).val(inst.color.toHex(true));
					}

					if (!jQuery('#ui-colorpicker-hex-alpha', e).is(':focus')) {
						jQuery('#ui-colorpicker-hex-alpha', e).val(inst._intToHex(inst.color.a * 255));
					}
				};
			...
			swatches: function (inst) {
				...
				_html = function () {
					...
					jQuery.each(inst.options.swatches, function (name, color) {
				...
				this.init = function () {
					e = jQuery(_html()).appendTo(jQuery('#ui-colorpicker-swatches-container', inst.dialog));

					jQuery('.ui-colorpicker-swatch', e).click(function () {
						var rgb		= inst._parseColor(jQuery(this).css('background-color'));
			...
			footer: function (inst) {
				...
				this.init = function () {
					e = jQuery(_html()).appendTo(inst.dialog);

					jQuery('.ui-colorpicker-close', e).button().click(function () {
						inst.close();
					});

					if (inst.options.alpha) {
						jQuery('.ui-colorpicker-transparent', e).button().click(function () {
							inst.color.a = 0;
							inst._change();
						});
					}
		...
		Color: function () {
			...
			this.normClone = function () {
				return jQuery.extend({}, this, {s: 1, v: 1}).updateRGB();
			};
